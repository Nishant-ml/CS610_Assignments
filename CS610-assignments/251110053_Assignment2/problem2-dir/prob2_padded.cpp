...
__attribute__((aligned(64))) struct word_tracker {
  uint64_t word_count[5*PAD_MULTIPLIER];
...
void print_counters() {
  for (int id = 0; id < MAX_THREADS; ++id) {
    std::cout << "Thread " << id << " counter: " << tracker.word_count[id*PAD_MULTIPLIER]
              << '\n';
  }
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < thread_count*PAD_MULTIPLIER; i++)
...
void *thread_runner(void *th_args) {
  ...
  while (getline(input_file, line)) {
    ...
    while ((pos = line.find(delimiter)) != std::string::npos) {
      ...
      tracker.word_count[thread_id*PAD_MULTIPLIER]++;
